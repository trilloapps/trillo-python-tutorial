...
def auditLog(logObject=None, type=None, summary=None, permitLogging=True, *args):
    if logObject is not None:
        if permitLogging:
            try:
                qualifiedAuditClass = Util.SHARED_APP_NAME + "." + Util.COMMON_DS_NAME + "." + "AuditLog"
                logObject["txId"] = _call_logger.txId
                res = DSApi.save(qualifiedAuditClass, logObject)
                if isinstance(res, dict) and res["__inserted__"] == True:
                    pass
                else:
                    logError("Error in saving audit log record")
            except Exception as e:
                logError("Error in saving audit log record : " + str(e))
