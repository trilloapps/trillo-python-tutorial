...
def testing(parameters):
    ...
    try:
        ...
        video_size = (int(video.size[0]), int(video.size[1]))
        resized_image = cv2.resize(imageCv, video_size, interpolation=cv2.INTER_LANCZOS4)

        LogApi.auditLogInfo("1")
        image = ImageClip(resized_image)
        LogApi.auditLogInfo("2")
